...
tipos de datos para cada constructor, y efectuar la clausura con una Typeclass.
...
> class HRLabelSet (ps :: [*])
> instance (HLabelSet (LabelsOf ps){-, HAllTaggedLV ps-})
>   => HRLabelSet (ps :: [*])


>
> type family LabelsOf (ls :: [Type]) :: [Type]
...
> mkRecord :: (HRLabelSet r{-, HAllTaggedLV r -}) => HList r -> Record r
> mkRecord = Record
...
>   => HasField l (Tagged l v ': r) v where
>     hLookupByLabel l r =
>              hLookupByLabel' (Proxy :: Proxy True) l r

> instance {-# OVERLAPPABLE #-} (HasField' False l (Tagged l1 v1 ': r) v)
>   => HasField l (Tagged l1 v1 ': r) v where
>     hLookupByLabel l r =
>              hLookupByLabel' (Proxy :: Proxy False) l r

>
> class HasField' (b::Bool) (l :: k) (r::[Type]) v
>   | b l r -> v where
>     hLookupByLabel':: Proxy b -> Label l -> Record r -> v

> instance HasField' True l (Tagged l v ': r) v where
>     hLookupByLabel' _ _ (Record (HCons (Tagged v) _)) = v

> instance HasField l r v
>   => HasField' False l (fld ': r) v where
>     hLookupByLabel' _ l (Record (HCons _ r)) = hLookupByLabel l (Record r)
> -}
...
---
Azucar sintactica para crear records

TODO: Quiero evitar toda la verborragia, estaria bueno tener una funcion
polivariadica que construya Record o bien una notacion parecida a
la azucar sintactica de listas..
